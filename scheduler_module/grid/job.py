...
from scheduler_module.grid import cell
...
class Job(pykka.ThreadingActor):
    ...
    def __init__(self, job_name, state: JobState, charging_point: cell, job_cells):
    ...
    def has_cp(self):
        if self.charging_point in self.job_cells:
            return True
        return False
    ...
    def get_first_cell(self):
        for i in range(len(self.job_cells)):
            if not self.job_cells[i].is_cell_explored():
